...
public class TestFingerprintFilter extends BaseTokenStreamTestCase {
  ...
  public void testBehavior() throws Exception {
    for (final boolean consumeAll : new boolean[] {true, false}) {
      ...
      TokenStream stream = new TokenConcatenateFilter(tokenizer);
      assertTokenStreamContents(stream, new String[] {"B2 A1 C3 D4 E5 F6 G7 H1"});
    }
  }
  ...
  public void testMaxFingerprintSize() throws Exception {
    for (final boolean consumeAll : new boolean[] {true, false}) {
      MockTokenizer tokenizer = whitespaceMockTokenizer("B2 A1 C3 D4 E5 F6 G7 H1");
      tokenizer.setEnableChecks(consumeAll);
      TokenStream stream = new TokenConcatenateFilter(tokenizer, 4, ' ');
  ...
  public void testCustomSeparator() throws Exception {
    for (final boolean consumeAll : new boolean[] {true, false}) {
      ...
      tokenizer.setEnableChecks(consumeAll);
      TokenStream stream =
          new TokenConcatenateFilter(tokenizer, TokenConcatenateFilter.DEFAULT_MAX_OUTPUT_TOKEN_SIZE, '_');
      assertTokenStreamContents(stream, new String[] {"B2_A1_C3_B2"});
    }
  }
